    ...
    >
      ...
      {(segment.segType === 'All' || segment.segType === 'Digital') && (
        <div className="flex flex-col gap-2">
           ...
           <button
            onClick={onToggle}
            className={`w-full py-3.5 rounded-lg font-black text-sm tracking-[0.2em] transition-all shadow-md flex items-center justify-center gap-3 border-b-2 active:border-b-0 active:translate-y-0.5
              ${isOn 
      ...
      {/* Proportional Control - Compacted */}
      {(segment.segType === 'All' || segment.segType === 'PWM') && (
        <div className="bg-gray-50 dark:bg-black/20 p-4 rounded-xl border border-gray-100 dark:border-white/5 flex flex-col gap-3">
          <Slider
            label="PWM Intensity"
      ...
      <div className="flex flex-col gap-2 bg-gray-50 dark:bg-black/20 p-4 rounded-xl border border-gray-100 dark:border-white/5">
        ...
        <div className="flex gap-2">
          <input 
            ...
            className="flex-1 bg-white dark:bg-black border border-gray-200 dark:border-white/10 rounded-lg px-3 py-1.5 text-[10px] font-mono dark:text-white outline-none focus:border-primary transition-colors"
          />
          <button className="bg-black text-primary p-1.5 rounded-lg hover:bg-primary hover:text-black transition-all">
            <Send size={14} />
          </button>
          <button 
            onClick={() => setCode("")}
            className="bg-gray-100 dark:bg-white/5 text-gray-400 p-1.5 rounded-lg hover:text-red-500 transition-all"
          >
            <Trash2 size={14} />
          </button>
        </div>
