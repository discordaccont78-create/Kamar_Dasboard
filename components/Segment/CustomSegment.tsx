        ...
        <div className="relative">
           <div className="flex justify-between items-center mb-1.5 md:mb-2 px-1">
              <label className="text-[9px] md:text-[10px] font-black text-muted-foreground uppercase tracking-widest flex items-center gap-1">
                 ...
                 {mode === 'toggle' ? t.toggle_mode : t.momentary_mode}
              </label>
              <button 
                onClick={cycleMode} 
           ...
           >
              <div className={cn("absolute inset-0 opacity-20 transition-opacity duration-500", isOn ? "bg-[url('https://grainy-gradients.vercel.app/noise.svg')] opacity-40" : "")} />
              <div className="flex items-center justify-center gap-3 relative z-10">
                 <Power className={cn("w-5 h-5 md:w-6 md:h-6 transition-all duration-300", isOn ? "text-primary drop-shadow-[0_0_8px_rgba(var(--primary),0.8)]" : "text-muted-foreground opacity-50")} />
                 <span className={cn("text-base md:text-xl font-black uppercase tracking-[0.2em] transition-colors duration-300", isOn ? "text-foreground" : "text-muted-foreground opacity-50")}>
                    {isOn ? t.active_status : t.offline_status}
                 </span>
              </div>
              <div className={cn("absolute bottom-0 left-0 h-1 transition-all duration-500 ease-out", isOn ? "w-full bg-primary shadow-[0_-2px_10px_rgba(var(--primary),0.5)]" : "w-0 bg-transparent")} />
           </button>
        </div>
      )}
      {showSlider && (
        <div className="bg-secondary/5 p-3 md:p-4 rounded-lg md:rounded-xl border border-border-light dark:border-border-dark flex flex-col gap-3 md:gap-4">
          <div className="flex justify-between items-center">
             <label className="text-[9px] md:text-[10px] font-bold text-muted-foreground uppercase tracking-widest">{t.set_pwm_val}</label>
             <span className="font-mono text-[10px] md:text-xs font-bold text-primary">{localPwm}</span>
          </div>
          <Slider value={[localPwm]} onValueChange={handleSliderChange} onValueCommit={handleSliderCommit} max={255} step={1} className="w-full" />
        </div>
      )}
      {showCode && (
        <div className="flex flex-col gap-2">
            <label className="text-[9px] text-muted-foreground font-black uppercase tracking-widest ml-1">{t.protocol_injector}</label>
            <div className="flex gap-2 items-center">
            <Input type="text" placeholder="HEX..." value={code} onChange={(e) => setCode(e.target.value)} className="h-9 text-xs" />
            <Button size="sm" className="h-9 px-3"><Send size={14} /></Button>
            <Button size="sm" variant="ghost" onClick={() => setCode("")} className="h-9 px-3 text-muted-foreground hover:text-destructive"><Trash2 size={14} /></Button>
            </div>
        </div>
      )}
      {showTimer && (
        <div className={cn("flex items-center justify-between px-1 transition-all duration-300", timeLeft !== null ? "bg-primary/10 p-1.5 md:p-2 rounded-lg border border-primary/30" : "")}>
            <div className="flex items-center gap-2 text-muted-foreground">
            {timeLeft !== null ? <Hourglass size={12} className="animate-spin text-primary" /> : <Clock size={12} />}
            <span className={cn("text-[8px] md:text-[9px] font-black uppercase tracking-widest", timeLeft !== null ? "text-primary" : "")}>
                {timeLeft !== null ? t.auto_action_in : t.runtime}
            </span>
            </div>
            {timeString ? (
